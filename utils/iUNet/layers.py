...
class OrthogonalResamplingLayer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.stride = stride
        self.dilate = dilate
        self.channel_multiplier = channel_multiplier# int(np.prod(stride))
        ...
        kernel_size = int((self.channel_multiplier * self.low_channel_number)**0.5)

        self._kernel_matrix_shape = ((self.low_channel_number,)
                                     + (self.low_channel_number * self.channel_multiplier,) * 2)
        self._kernel_shape = ((self.high_channel_number, self.low_channel_number)
    ...
    def kernel_matrix_transposed(self):
        """The orthogonal matrix created by the chosen parametrisation method.
        """
        kernel_matrix = self.__calculate_kernel_matrix__(self.weight, **self.kwargs)

        # print(kernel_matrix)
        # print(torch.flip(kernel_matrix, [2]))

        # print(kernel_matrix.size())
        # print(torch.transpose(kernel_matrix, -1, -2).size())
        return torch.flip(kernel_matrix, [2])

    @property
    def kernel_transposed(self):
        """The kernel associated with the invertible up-/downsampling.
        """
        # print(self.kernel_matrix.size())
        return self.kernel_matrix_transposed.reshape(*self._kernel_transposed_shape)
...
class InvertibleDownsampling2D(OrthogonalResamplingLayer):
    def __init__(self,
        ...
        self.in_channels = in_channels
        self.out_channels = in_channels * channel_multiplier
        self.low_channel_numbers = self.in_channels
        self.dilate = dilate
        # print(stride)
    ...
    def get_transposed_kernel(self):
        return self.kernel_matrix_transposed
...
def __initialize_weight__(kernel_matrix_shape: Tuple[int, ...],
    ...
    if init is 'dct':
        dct = dct_matrix(stride[0]**2)
        I = torch.eye(dct.shape[-1], device=dct.device)
        LU = torch.lu(I + dct, pivot=True)
        skew_sym = torch.lu_solve(I - dct, *LU)
        weight = torch.triu(skew_sym, diagonal=1)

        # return torch.tensor(weight).repeat(num_matrices, 1, 1)
        return (weight.clone().detach()).repeat(num_matrices, 1, 1)
